...
class WPEM_Updater {
	...
	public function activation_links( $links ) {
		$links[] = '<a href="' . add_query_arg( array('post_type' => 'event_listing', 'page' => 'license'), admin_url( 'edit.php' ) ) . '">' . __('Activate licence', 'wpem-pluginslug') . '</a>';
		return $links;
	}
	...
	public function deactivation_links( $links ) {
		$links[] = '<a href="' . remove_query_arg( array( 'deactivated_licence', 'activated_licence' ), add_query_arg( $this->plugin_slug . '_deactivate_licence', 1 ) ) . '">' . __('Deactivate licence', 'wpem-pluginslug') . '</a>';
		return $links;
	}
	...
	public function handle_errors( $errors ) {
		if ( !empty( $errors['no_key'] ) ) {
			$this->add_error( sprintf( __('A licence key for %1$s could not be found. Maybe you forgot to enter a licence key when setting up %2$s.', 'wpem-pluginslug'), esc_html( $this->plugin_data['Name'] ), esc_html( $this->plugin_data['Name'] ) ) );
